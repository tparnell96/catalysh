...
pub fn handle_eox_command(subcommand: EoxCommands) {
    command_utils::execute_with_context(|ctx| async move {
        match subcommand {
            EoxCommands::Summary => match eox::get_eox_summary(&ctx.config, &ctx.token).await {
                Ok(resp) => {
                        ...
                        if let Some(devices) = resp.response {
                            if crate::helpers::output::is_json() {
                                ...
                                for d in devices {
                                    ...
                                    table.add_row(row![
                                        "EoSW Maintenance Date",
                                        d.eo_sw_maintenance_releases_date
                                            .as_deref()
                                            .unwrap_or("N/A")
                                    ]);
                                    table.add_row(row![
                                        "EoSecurity Vuln Support",
                                        d.eo_security_vuln_support_date.as_deref().unwrap_or("N/A")
                                    ]);
                                    table.add_row(row![
                                        "EoService Contract Renewal",
                                        d.eo_service_contract_renewal_date
                                            .as_deref()
                                            .unwrap_or("N/A")
                                    ]);
                                    table.add_row(row![
                                        "EoLast HW Ship Date",
                                        d.eo_last_hw_ship_date.as_deref().unwrap_or("N/A")
                                    ]);
                                    table.add_row(row![
                                        "Comments",
                                        d.comments.as_deref().unwrap_or("N/A")
                                    ]);
                                }
                                table.printstd();
                            }
